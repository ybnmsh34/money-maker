...
(async () => {
    ...
    // Inject debug version of splitSpanAtCursor
    await page.evaluate(() => {
        const origSplit = window.splitSpanAtCursor;
        // Override with debug version
        window._debugSplitCount = 0;
    });
    ...
    const result = await page.evaluate(() => {
        ...
        return {
            ...
            spansPerPage,
            lastPageId: lastPage ? lastPage.id : 'none',
            lastPageH: lastPage ? lastPage.offsetHeight : null,
            lastPageSpans: lastPage ? Array.from(lastPage.querySelectorAll('.text-span')).map(s => ({
    ...
    await page.screenshot({ path: 'C:/pi-agent/money-maker/test-png/test-debug-shift4.png', fullPage: false });
    await browser.close();
})();
