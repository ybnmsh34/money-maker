...
(async () => {
    ...
    const result = await page.evaluate(() => {
        ...
        document.querySelectorAll('.text-span').forEach(s => {
            ...
            if (!spansPerPage[p]) spansPerPage[p] = [];
            spansPerPage[p].push({ text: s.textContent.substring(0,30), top: parseFloat(s.style.top) });
        });
        return {
            totalPages: pages.length,
            spansPerPage: Object.fromEntries(Object.entries(spansPerPage).map(([k,v]) => [k, v.length])),
        };
    });
