...
(async () => {
    ...
    page.on('console', msg => {
        const t = msg.text();
        if (t.includes('OVERFLOW') || t.includes('DEBUG2')) console.log('CONSOLE:', t);
    });
    ...
    // Add debug logging to see how many spans are past bottom
    await page.evaluate(() => {
        const origShift = shiftSpansDown;
        window.shiftSpansDown = function(pageDiv, startY, shiftAmount, pageNum) {
            const result = origShift(pageDiv, startY, shiftAmount, pageNum);
            const pageH = pageDiv.offsetHeight;
            const allSpans = Array.from(pageDiv.querySelectorAll('.text-span'));
            const pastBottom = allSpans.filter(s => {
                const top = parseFloat(s.style.top);
                const h = s._meta ? s._meta.height * zoom : 13 * zoom;
                return top + h > pageH - 10;
            });
            console.log('DEBUG2:', 'startY=' + startY, 'shiftAmount=' + shiftAmount, 'overflowCount=' + result.length, 'pastBottomCount=' + pastBottom.length, 'totalSpans=' + allSpans.length);
            return result;
        };
    });
